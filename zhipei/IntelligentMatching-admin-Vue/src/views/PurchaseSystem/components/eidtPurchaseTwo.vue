<template>
    <div>
		...
		<el-row class="mainContentItemBox">
			...
			<el-table :data="tableData" style="width: 100%" align="center">
				...
				<el-table-column label="数量" width="120">
					<template slot-scope="scope">
						<el-input v-model="scope.row.goods_num" v-if="editType" @blur="/[^\d]/g.test(scope.row.goods_num) ? scope.row.goods_num = 0:'' " placeholder="数量" max="10" :maxlength="10" clearable class="mainIptSelBox" />
                        <p v-else>{{scope.row.goods_num}}</p>
					</template>
				</el-table-column>
				<el-table-column label="单位" prop="company_unit" />
				<el-table-column label="单价" width="120">
					<template slot-scope="scope">
						<el-input v-model="scope.row.univalence" v-if="editType" @blur="!/(^[1-9]\d*(\.\d{1,2})?$)|(^0(\.\d)?$)/.test(scope.row.univalence) ? scope.row.univalence = 0 :''" placeholder="单价" max="10" :maxlength="10" clearable class="mainIptSelBox" />
                        <p v-else>{{scope.row.univalence}}</p>
					</template>
				</el-table-column>
				<el-table-column width="120px" label="税率">
					<template slot-scope="scope">
						<el-select v-model="scope.row.tax_rate" class="mainIptSelBox" @change="bumens" v-if="editType">
                        ...
                        <p v-else>{{scope.row.tax_rate}}%</p>
					</template>
				</el-table-column>
				<el-table-column width="150px" label="币种">
					<template slot-scope="scope">
						<el-select v-model="scope.row.currency"  class="mainIptSelBox" v-if="editType">
				...
				<el-table-column width="200px" label="预计到货时间">
					<template slot-scope="scope">
						<el-date-picker style="width:150px" v-model="scope.row.estimate_time" align="right" type="date" placeholder="选择日期" :picker-options="pickerOptions">
						</el-date-picker>
					</template>
				</el-table-column>
				<el-table-column label="操作" fixed="right" width="150" align="center">
					<template slot-scope="scope">
						<el-button size="mini" v-if="editType" plain @click="deltaiFun(scope.$index)" type="danger">撤销</el-button>
					</template>
				</el-table-column>
			</el-table>
		...
		<div class="bottomBtn">
			总金额	
			<el-input v-model="moneys" placeholder=""  @blur="!/(^[1-9]\d*(\.\d{1,2})?$)|(^0(\.\d)?$)/.test(moneys) ? moneys = 0 :''" style="width: 100px; margin-left: 10px;"/><span style="color: red;">	*</span>
...
<script>
import {PurchasePlan_turnPurchase} from '@/api/PurchaseSystem'
...
export default {
    ...
    methods:{
        ...
        getinforData(){
            ...
            PurchasePlan_turnPurchase(datadd).then(success=>{
                loading.close();
