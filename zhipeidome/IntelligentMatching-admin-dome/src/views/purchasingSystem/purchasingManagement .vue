	...
	<div>
		...
		<el-row class="mainContentItemBox" v-if="deliveryList">
				...
				<div class="orhead-orbi_state">
					...
					<div class="sd-module-btn">
						...
						<div :class="formList.state === 1?'el-tab-order on':' el-tab-order'" @click="distinguish(1)">
							待处理(<span style="color:red;">0</span>)
						</div>
						<div :class="formList.state === 2?'el-tab-order on':' el-tab-order'" @click="distinguish(1)">
							审核中(<span style="color:red;">0</span>)
						</div>
						<div :class="formList.state === 3?'el-tab-order on':' el-tab-order'" @click="distinguish(3)">
							待入库(<span style="color:red;">4</span>)
						</div>
			...
			<el-form ref="formList" :model="formList" size="medium" label-width="100px" class="mainSearchItemBox">
				<el-row>
					...
					<el-col :span="6">
						<el-form-item label="采购状态:" prop="bra_name3">
					...
					<el-col :span="6">
						<el-form-item label="结算状态:" prop="bra_name7">
		...
		<el-row class="mainContentItemBox" v-if="deliveryList">
			...
			<el-table :data="tableData" style="width: 100%">
				...
				<el-table-column label="采购单状态" prop="numbers9" />
				<el-table-column label="结算状态" prop="numbers10" />
				<el-table-column label="操作" width="260" align="center">
					<template slot-scope="scope">
						<el-button size="mini" plain type="primary" v-if="scope.row.numbers9 == '待处理'">提交审核</el-button>
						<el-button size="mini" plain type="primary" v-if="scope.row.numbers9 == '审核中'" @click="addBrandBtn3()">审核</el-button>
						<el-button size="mini" plain type="primary" v-if="scope.row.numbers9 == '待处理'" @click="addBrandBtn2()">编辑</el-button>
					</template>
				</el-table-column>
			</el-table>
		...
		<!--<<添加采购单-->
		<!--<<编辑-->
		<eidtPurchase v-if="deliveryMain1" @listerToChild="postOrderListMsg1()" />
		<!--<<编辑-->
		<!--<<审核-->
		<eidtPurchas v-if="deliveryMain2" @listerToChild="postOrderListMsg1()" />
		<!--<<审核-->
	</div>
</template>
<script>
	...
	import eidtPurchas from './components/eidtPurchas'; //送货单
	export default {
		components: {
			addPurchase,
			eidtPurchas,
