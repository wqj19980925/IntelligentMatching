	...
	<div>
		...
		<el-row class="mainContentItemBox" v-if="deliveryList">
			...
			<el-form ref="formList" :model="formList" size="medium" label-width="100px" class="mainSearchItemBox">
				<el-row>
					...
					<el-col :span="6">
						<el-form-item label="入库单号:" prop="bra_name5">
							<el-input v-model="formList.bra_name5" placeholder="请输入入库单号" max="10" :maxlength="10" clearable class="mainIptSelBox" />
						</el-form-item>
					</el-col>

					<el-col :span="6">
						<el-form-item label="入库状态:" prop="bra_name3">
		...
		<el-row class="mainContentItemBox" v-if="deliveryList">
			...
			<el-table :data="tableData" style="width: 100%">
				<el-table-column type="expand">
					<template slot-scope="props">
						<el-table :data="props.row.tableLastData" style="width: 100%">
							...
							<el-table-column label="采购数量" prop="numbermain5" />
							<el-table-column label="入库数量" prop="numbermain6" />
							<el-table-column label="单位" prop="numbermain8" />
							<el-table-column label="预计到货时间" prop="numbermain12" />
							<el-table-column label="实际入库时间" prop="numbermain7" />
						</el-table>
					</template>
				</el-table-column>
				<el-table-column width='200px' label="采购单号" prop="numbers" />
				<el-table-column width='300px' label="供货单位" prop="numbers1" />
				<el-table-column label="采购总数" prop="numbers3" />
				<el-table-column label="到货总数" prop="numbers2" />
				...
				<el-table-column label="入库单状态" prop="numbers9" />
				<el-table-column label="操作" width="380" align="center">
					<template slot-scope="scope">
						<el-button size="mini" plain type="primary" v-if="scope.row.numbers9 != '已完成'" @click="addBrandBtn()">入库</el-button>
						<el-button size="mini" plain type="primary" v-if="scope.row.numbers9 != '未入库'" @click="addBrandBtns()">记录</el-button>
						<el-button size="mini" plain type="primary" v-if="scope.row.numbers9 != '未入库'" @click="addBrandBtns3()">退货</el-button>
						<el-button size="mini" plain type="primary" v-if="scope.row.numbers9 != '未入库'" @click="addBrandBtns4()">换货</el-button>
					</template>
				</el-table-column>
			</el-table>
		...
		<!--入库记录>>-->
		<!--<<退货-->
		<addPurchasetwo v-if="deliveryMain3" @listerToChild="postOrderListMsg3()" />
		<!--退货>>-->
		<!--<<换货-->
		<addPurchasetree v-if="deliveryMain4" @listerToChild="postOrderListMsg4()" />
		<!--换货>>-->
...
<script>
	...
	import addPurchasetwo from './components/addPurchasetwo'; //送货单
	import warehousingList from './components/warehousingList'; //送货单
	import addPurchasetree from './components/addPurchasetree'; //换货
	export default {
		components: {
			addPurchase,
			warehousingList,
